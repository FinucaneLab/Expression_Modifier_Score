...
#the process is actually included in the train_a_predictor.py, but
#for now, doing it separately
...
subprocess.call(["gsutil", "cp" ,"gs://qingbowang/ems_v1_test/grid_search_result_{0}.tsv".format(leftout_chr), "./"])
...
#filter to leave out one chromosome:
...
dfgrid = pd.read_csv("grid_search_result_{0}.tsv".format(leftout_chr), sep="\t", index_col=0)
...
#save
import pickle
filename = 'ems_rfmodel_{0}_leftout_{1}.sav'.format(tissue_name, leftout_chr)
pickle.dump(regr, open(filename, 'wb'))


# do the scoring 1000 times, 90% training + 10% test data
#actuallly 100 times seems fine, as long as we do proper smoothing later
...
ns = pd.read_csv("n_pos_and_neg_per_leftout_chr.tsv", sep="\t", index_col=0)
n_pos = ns.loc[leftout_chr,"n_pos"]
n_neg = ns.loc[leftout_chr,"n_neg"] #actually having a threshold here seems fine.
...
#save
p_causal.to_csv("ems_p_causal_{0}_leftout_{1}.tsv".format(tissue_name, leftout_chr), sep="\t")

subprocess.call(["gsutil", "cp" ,"./ems_rfmodel_{0}_leftout_{1}.sav".format(tissue_name, leftout_chr) ,"gs://qingbowang/ems_v1_test/ems_rfmodel_{0}_leftout_{1}.sav".format(tissue_name, leftout_chr)])
subprocess.call(["gsutil", "cp", "./ems_p_causal_{0}_leftout_{1}.tsv".format(tissue_name, leftout_chr), "gs://qingbowang/ems_v1_test/ems_p_causal_{0}_leftout_{1}.tsv".format(tissue_name, leftout_chr)])


#and remove the files:
subprocess.call(["rm", "./grid_search_result_{0}.tsv".format(leftout_chr)])
