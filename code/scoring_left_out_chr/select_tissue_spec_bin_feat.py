...
for m in marks:
    ...
    ##filter to my chr of interest
    pos = pos[pos.vg.str.split("_").str[0]!=leftout_chr]
    neg = neg[neg.vg.str.split("_").str[0] != leftout_chr]
...
#df["LOR"] = np.log2(df.OR) #= how much it is enriched in the positive set

#and save
with hl.hadoop_open("gs://qingbowang/ems_v1_test/{0}_roadmapannot_f1measures_{1}_leftout.tsv".format(tissue_name, leftout_chr), 'w') as f:
    df.to_csv(f, sep="\t")
...
pos = pos.filter(pos.variant_id.split("_")[0]!=leftout_chr)#filter to other chrs
...
neg = neg.filter(neg.variant_id.split("_")[0]!=leftout_chr)#filter to other chrs
...
#df["LOR"] = np.log2(df.OR) #= how much it is enriched in the positive set
#and save
with hl.hadoop_open("gs://qingbowang/ems_v1_test/{0}_tissue_union_annot_f1measures_{1}_leftout.tsv".format(tissue_name, leftout_chr), 'w') as f:
    df.to_csv(f, sep="\t")
