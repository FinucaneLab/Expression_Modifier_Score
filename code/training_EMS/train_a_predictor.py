...
mfeat_grid = np.arange(0.5, min(2/mfeat, 1)+0.01, 0.1) * mfeat #should be min(1/mfeat, 2) ..
...
#save the best one
dfgrid.sort_values(by="auroc", ascending=False).to_csv("grid_search_result_{0}.tsv".format(tissue_name), sep="\t")
df.sort_values(by="auroc", ascending=False).to_csv("random_search_result_{0}.tsv".format(tissue_name), sep="\t")
